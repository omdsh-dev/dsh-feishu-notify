 ...
 */
export function isSubagentTarget(target: NotifyTarget): boolean {
...
export function lastTwoDirs(cwd: string): string {
...
export async function sendFeishu(
...
): Promise<{ code?: number; StatusCode?: number } | null> {
  ...
  try {
    ...
    console.error(`[dsh-feishu-notify] error: ${(e as Error).message}`)
    return null
  } finally {
    clearTimeout(timer)
  }
 ...
 */
export async function notify(config: NotifyConfig, type: NotifyType, target: NotifyTarget): Promise<void> {
  try {
    if (isSubagentTarget(target)) return
    if (!config.enabled || !config.webhookUrl) return
    const cwd = target.cwd || process.cwd()
    await sendFeishu(config, buildCard(type, cwd, target.sessionId, config.webUrl))
  } catch (e) {
    console.error(`[dsh-feishu-notify] ${type} error: ${(e as Error).message}`)
  }
}
